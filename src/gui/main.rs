...
mod feature {
    ...
    pub fn main() {
        ...
        'main: loop {
            ...
            if arena_borrow.chainables[&fft_id].read().unwrap().output().len() > 0
            {
                println!("FFT 0: {}", arena_borrow.chainables[&fft_id].read().unwrap().output()[0]);
            }
            ...
            set_ui(ui.set_widgets(), &ids, &fonts, arena_borrow.chainables[&fft_id].read().unwrap().output());
    ...
    fn set_ui(ref mut ui: conrod::UiCell, ids: &Ids, fonts: &Fonts, points: &Vec<f32>) {
        ...
        widget::Grid::new(min_x, max_x, min_y, max_y, lines.iter().cloned())
            ...
            .set(ids.grid, ui);
        widget::PlotPath::new(min_x, max_x, min_y, max_y, |mut x|  {
                if points.len() > 0 {
                    x = (x*x)/2.0;
                    let mut value = points[(x * (points.len() as f64/max_x) as f64) as usize] as f64;
                    value = value * value;
                    return value;
                }
                else {
